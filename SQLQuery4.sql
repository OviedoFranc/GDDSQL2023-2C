...
CREATE PROC MigrarInmueble AS
        ...
        INSERT INTO DATA_TEAM.Barrio (BARRIO)
            ...
            WHERE INMUEBLE_BARRIO IS NOT NULL AND 
                NOT EXISTS (SELECT 1 FROM DATA_TEAM.Barrio b WHERE b.BARRIO = gd_esquema.Maestra.INMUEBLE_BARRIO); 

        INSERT INTO DATA_TEAM.Localidad (LOCALIDAD)
            SELECT DISTINCT INMUEBLE_LOCALIDAD
            FROM gd_esquema.Maestra
            WHERE INMUEBLE_LOCALIDAD IS NOT NULL AND 
                NOT EXISTS (SELECT 1 FROM DATA_TEAM.Localidad l WHERE l.LOCALIDAD = gd_esquema.Maestra.INMUEBLE_LOCALIDAD);
...
BEGIN
    ...
    INSERT INTO DATA_TEAM.Inmueble_Caracteristica (INMUEBLE_ID, CARACTERISTICA)
    SELECT DISTINCT
        m.INMUEBLE_CODIGO,
        CASE 
...
CREATE PROC MigrarAlquiler AS
    ...
    INSERT INTO DATA_TEAM.Tipo_periodo (TIPO_PERIODO)                   
        ...
        FROM gd_esquema.Maestra
        WHERE ANUNCIO_TIPO_PERIODO IS NOT NULL AND 
            NOT EXISTS (SELECT 1 FROM DATA_TEAM.Tipo_periodo m WHERE m.Tipo_periodo = gd_esquema.Maestra.ANUNCIO_TIPO_PERIODO);
...
CREATE TABLE DATA_TEAM.Inmueble (
    ...
    INMUEBLE_LOCALIDAD NVARCHAR(100) FOREIGN KEY REFERENCES DATA_TEAM.Localidad, -- FK
    INMUEBLE_PROVINCIA NVARCHAR(100) FOREIGN KEY REFERENCES DATA_TEAM.Provincia, -- FK
