...
CREATE PROC MigrarInmueble AS
        ...
        INSERT INTO DATA_TEAM.Barrio (BARRIO, LOCALIDAD)
            SELECT DISTINCT INMUEBLE_BARRIO, INMUEBLE_LOCALIDAD
            FROM gd_esquema.Maestra
            WHERE INMUEBLE_BARRIO IS NOT NULL AND INMUEBLE_LOCALIDAD IS NOT NULL AND
                NOT EXISTS (SELECT 1 FROM DATA_TEAM.Barrio b WHERE b.BARRIO = gd_esquema.Maestra.INMUEBLE_BARRIO AND b.LOCALIDAD = gd_esquema.Maestra.INMUEBLE_LOCALIDAD); 

        INSERT INTO DATA_TEAM.Localidad (LOCALIDAD, PROVINCIA)
            SELECT DISTINCT INMUEBLE_LOCALIDAD, INMUEBLE_PROVINCIA
            FROM gd_esquema.Maestra
            WHERE INMUEBLE_LOCALIDAD IS NOT NULL AND INMUEBLE_PROVINCIA IS NOT NULL AND
                NOT EXISTS (SELECT 1 FROM DATA_TEAM.Localidad l WHERE l.LOCALIDAD = gd_esquema.Maestra.INMUEBLE_LOCALIDAD AND l.PROVINCIA = gd_esquema.Maestra.INMUEBLE_PROVINCIA);
...
CREATE PROC MigrarSucursal AS
    ...
    INSERT INTO DATA_TEAM.Localidad (LOCALIDAD)
        SELECT DISTINCT SUCURSAL_LOCALIDAD
        FROM gd_esquema.Maestra
        WHERE SUCURSAL_LOCALIDAD IS NOT NULL AND 
            NOT EXISTS (SELECT 1 FROM DATA_TEAM.Localidad l WHERE l.LOCALIDAD = gd_esquema.Maestra.SUCURSAL_LOCALIDAD);
...
CREATE TABLE DATA_TEAM.Localidad (
    LOCALIDAD NVARCHAR(100) PRIMARY KEY,
	PROVINCIA NVARCHAR(100) FOREIGN KEY REFERENCES DATA_TEAM.Provincia
);
...
CREATE TABLE DATA_TEAM.Barrio(
	...
	LOCALIDAD NVARCHAR(100) FOREIGN KEY REFERENCES DATA_TEAM.Localidad
)
...
CREATE TABLE DATA_TEAM.Sucursal (
    ...
    SUCURSAL_LOCALIDAD NVARCHAR(100) FOREIGN KEY REFERENCES DATA_TEAM.Localidad, -- FK
...
CREATE TABLE DATA_TEAM.Inmueble (
    ...
    INMUEBLE_BARRIO NVARCHAR(100) FOREIGN KEY REFERENCES DATA_TEAM.Barrio, -- FK
