...
ORDER BY
	...
	TOTAL_ALTAS_ALQUILER
GO
/*Msg 4511, Level 16, State 1, Procedure Top5BarriosAlquiler, Line 2 [Batch Start Line 578]
...
GO
CREATE VIEW DATA_TEAM.PorcentajePromediosIncrementoAlquileres AS
	SELECT 
		A.ALQUILER_ANIO AS Anio,
		A.ALQUILER_MES AS Mes,
	...
	FROM DATA_TEAM.BI_H_ALQUILER A
	JOIN DATA_TEAM.Pago_alquiler PA_ACTUAL ON YEAR(PA_ACTUAL.PAGO_ALQUILER_FEC_INI) = A.ALQUILER_ANIO AND MONTH(PA_ACTUAL.PAGO_ALQUILER_FEC_INI) = A.ALQUILER_MES
	JOIN DATA_TEAM.Pago_alquiler PA_ANTERIOR ON PA_ACTUAL.PAGO_ALQUILER_CODIGO = PA_ANTERIOR.PAGO_ALQUILER_CODIGO
		AND YEAR(PA_ANTERIOR.PAGO_ALQUILER_FEC_INI) = YEAR(DATEADD(MONTH, -1, PA_ACTUAL.PAGO_ALQUILER_FEC_INI))
		AND MONTH(PA_ANTERIOR.PAGO_ALQUILER_FEC_INI) = MONTH(DATEADD(MONTH, -1, PA_ACTUAL.PAGO_ALQUILER_FEC_INI))
	JOIN DATA_TEAM.Alquiler ALQ ON PA_ACTUAL.PAGO_ALQUILER_CODIGO = ALQ.ALQUILER_CODIGO
	WHERE 
		ALQ.ALQUILER_ESTADO = 'Alquilado' -- Alquileres activos
...
*/
 GO	
CREATE VIEW DATA_TEAM.PrecioPromedioM2 AS
	SELECT 
		Vta.VENTA_TIPO_INMUEBLE AS TIPO_INMUEBLE,
		Vta.VENTA_LOCALIDAD AS LOCALIDAD,
		Vta.VENTA_CUATRI AS CUATRIMESTRE,
		Vta.VENTA_ANIO AS ANIO,
	    AVG(i.INMUEBLE_SUPERFICIETOTAL) AS PRECIO_PROMEDIO_M2
	FROM 
		DATA_TEAM.BI_H_VENTA Vta
	JOIN 
		DATA_TEAM.Venta v ON DATA_TEAM.Calcular_cuatri(v.VENTA_FECHA) = Vta.VENTA_CUATRI AND YEAR(v.VENTA_FECHA) = Vta.VENTA_ANIO
	JOIN DATA_TEAM.Anuncio a ON a.ANUNCIO_CODIGO = v.VENTA_ANUNCIO 
	JOIN DATA_TEAM.Inmueble i ON a.ANUNCIO_INMUEBLE = i.INMUEBLE_CODIGO
	WHERE
		Vta.VENTA_TIPO_INMUEBLE = i.INMUEBLE_TIPO_INMUEBLE 
			AND Vta.VENTA_LOCALIDAD = i.INMUEBLE_LOCALIDAD
GROUP BY 
    Vta.VENTA_TIPO_INMUEBLE,
    Vta.VENTA_LOCALIDAD,
    Vta.VENTA_CUATRI,
    Vta.VENTA_ANIO 
...
GO
CREATE VIEW DATA_TEAM.PromedioComision AS
    SELECT 
        CASE 
        ...
        AVG(
            CASE 
                WHEN V.TOTAL_COMISION IS NOT NULL THEN V.TOTAL_COMISION
                WHEN A.ALQUILER_COMISION IS NOT NULL THEN A.ALQUILER_COMISION
                ELSE 0 
            END
        ) AS Promedio_Comision
    FROM 
        DATA_TEAM.BI_H_VENTA V
    FULL JOIN 
        DATA_TEAM.BI_H_ALQUILER A ON V.VENTA_SUCURSAL = A.ALQUILER_SUCURSAL
            AND V.VENTA_ANIO = A.ALQUILER_ANIO
            AND V.VENTA_CUATRI = A.ALQUILER_CUATRI
    JOIN 
        DATA_TEAM.Anuncio AN ON AN.ANUNCIO_CODIGO = COALESCE(V.VENTA_ANUNCIO, A.ALQUILER_ANUNCIO)       ------------ No se como conectarlo con venta o alquiler, revisar
    GROUP BY 
        CASE 
            WHEN V.VENTA_TIPO_OPERACION IS NOT NULL THEN 'Venta'
            WHEN A.ALQUILER_TIPO_OPERACION IS NOT NULL THEN 'Alquiler'
